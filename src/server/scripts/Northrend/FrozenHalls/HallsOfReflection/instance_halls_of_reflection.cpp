...
public:
    ...
    struct instance_halls_of_reflection_InstanceMapScript : public InstanceScript
    {
        ...
        {
        	if (uiTeamInInstance == TEAM_NEUTRAL)
        	{
        		uiTeamInInstance = player->GetTeamId();
        		UpdateNpcFaction();
        	}

        }

        void UpdateNpcFaction()
        {
        	switch (uiTeamInInstance)
        	{
				case TEAM_ALLIANCE:
					if (Creature* sylvanas1 = instance->GetCreature(uiSylvanasPart1))
						sylvanas1->SetVisible(false);
					if (Creature* sylvanas2 = instance->GetCreature(uiSylvanasPart2))
						sylvanas2->SetVisible(false);
					if (Creature* loralen = instance->GetCreature(uiLoralen))
						loralen->SetVisible(false);
					break;
				case TEAM_HORDE:
					if (Creature* jaina1 = instance->GetCreature(uiJainaPart1))
						jaina1->SetVisible(false);
					if (Creature* jaina2 = instance->GetCreature(uiJainaPart2))
						jaina2->SetVisible(false);
					if (Creature* koreln = instance->GetCreature(uiKoreln))
						koreln->SetVisible(false);
					break;
				case TEAM_NEUTRAL:
				default:
					sLog->outString("Hall Of Reflection ERROR ! team in instance is neutral !");
					break;
        	}
        }
        ...
        void OnCreatureCreate(Creature* creature)
        {
            ...
            switch(creature->GetEntry())
            {
                ...
                case NPC_JAINA_PART1:
                    if (uiTeamInInstance == TEAM_HORDE)
                        creature->SetVisible(false);
                    uiJainaPart1 = creature->GetGUID();
                    break;
                case NPC_JAINA_PART2:
                	if (GetData(DATA_FROSWORN_EVENT) != DONE || uiTeamInInstance == TEAM_HORDE)
                		creature->SetVisible(false);
                	else if (GetData(DATA_FROSWORN_EVENT) == DONE)
                	   creature->SetVisible(true);
                	creature->SetHealth(252000);
                	uiJainaPart2 = creature->GetGUID();
                    break;
                ...
                case NPC_SYLVANAS_PART2:
                	if (GetData(DATA_FROSWORN_EVENT) != DONE || uiTeamInInstance == TEAM_ALLIANCE)
                		creature->SetVisible(false);
                	else if (GetData(DATA_FROSWORN_EVENT) == DONE)
                		creature->SetVisible(true);
					creature->SetHealth(252000);
					uiSylvanasPart2 = creature->GetGUID();
					break;
                case NPC_KORELN:
                    if (uiTeamInInstance == TEAM_HORDE)
                        creature->SetVisible(false);
        ...
        void SetData(uint32 type, uint32 data)
        {
            ...
            switch(type)
            {
                ...
                case DATA_FALRIC_EVENT:
                    uiEncounter[0] = data;
                    if (data == DONE)
                        events.ScheduleEvent(EVENT_NEXT_WAVE, 30000);
                    break;
                ...
                case DATA_FROSWORN_EVENT:
                    ...
                    if (data == DONE)
                    {
                        ...
                        if (Creature* Lk = instance->GetCreature(uiLichKing))
                        	Lk->SetVisible(true);
                        if (Creature* lidier = instance->GetCreature(GetData64(DATA_ESCAPE_LIDER)))
                        	lidier->SetVisible(true);
                    }
                    break;
                case DATA_LICHKING_EVENT:
                    ...
                    if(data == FAIL)
                    {
                        ...
                        if(Creature* pLider = instance->GetCreature(GetData64(DATA_ESCAPE_LIDER)))
                            pLider->Respawn(true);
        ...
        uint32 GetData(uint32 type)
        {
            switch(type)
            {
                ...
                case DATA_TEAM_IN_INSTANCE:     return uint32(uiTeamInInstance);
                ...
                case DATA_WAVE_EVENT: 			return uiWaveEvent;
        ...
        uint64 GetData64(uint32 identifier)
        {
            switch(identifier)
            {
                ...
                case DATA_ESCAPE_LIDER:
                	if (uiTeamInInstance == TEAM_HORDE && uiSylvanasPart2)
                		return uiSylvanasPart2;
                	if (uiTeamInInstance == TEAM_ALLIANCE && uiJainaPart2)
                		return uiJainaPart2;
                	return 0;
                ...
                case GO_CAPTAIN_CHEST_1:
                case GO_CAPTAIN_CHEST_3:		return uiChest[1];
                case GO_CAPTAIN_CHEST_2:
                case GO_CAPTAIN_CHEST_4:        return uiChest[0];
                case GO_SKYBREAKER:
        ...
        void AddWave()
        {
        	if (uiWaveCount <= 5 && GetData(DATA_FALRIC_EVENT) == DONE)
        	   uiWaveCount = 6;
