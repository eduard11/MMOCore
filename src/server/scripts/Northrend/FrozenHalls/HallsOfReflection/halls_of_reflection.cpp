            ...
            // Remaining Intro Events common for both faction
                ...
                case EVENT_INTRO_END:
                    if (pInstance)
                    {
                        ...
                        pInstance->SetData(DATA_WAVE_EVENT, SPECIAL);   // start first wave
                    }
...
public:
    ...
    struct npc_jaina_and_sylvana_hor_part2AI : public npc_escortAI
    {
        ...
        void Intro()
        {
            switch(Step)
            {
                ...
                case 7:
                    ...
                    if(pLichKing && !pLichKing->HasAura(SPELL_ICE_PRISON_VISUAL) && me->GetEntry() == NPC_JAINA_PART2)
                        pLichKing->AddAura(SPELL_ICE_PRISON_VISUAL, pLichKing);
                    if (pLichKing && !pLichKing->HasAura(SPELL_DARK_ARROW) && me->GetEntry() == NPC_SYLVANAS_PART2)
                    	pLichKing->AddAura(SPELL_DARK_ARROW, pLichKing);
        ...
        void Outro()
        {
            switch(Step)
            {
                case 10:
                    ...
                    break;
                case 11:
                    if(GameObject* pGunship = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(me->GetEntry() == NPC_JAINA_PART2 ? GO_SKYBREAKER : GO_ORGRIM_HAMMER)))
        ...
        {
            if(!m_pInstance || !Event || !me->IsVisible())
                return;
                ...
                switch(Count)
                {
                    ...
                    case 5:
                        if(pLichKing && pLichKing->isAlive())
                        {
                            ...
                            pLichKing->RemoveUnitMovementFlag(MOVEMENTFLAG_WALKING);
                            pLichKing->SetSpeed(MOVE_RUN, 1.0f, true);
                            Step = 0;
                        }
                        break;
                }
...
class npc_hor_waves_restarter : public CreatureScript
{
    public:
	npc_hor_waves_restarter() : CreatureScript("npc_hor_waves_restarter") {}

	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_hor_waves_restarterAI(creature);
	}

	struct npc_hor_waves_restarterAI: public ScriptedAI
	{
		npc_hor_waves_restarterAI(Creature *c) : ScriptedAI(c)
		{
		}
		...
		void MoveInLineOfSight(Unit* who)
		{
			...
			sLog->outString("See : %s", who->GetName());
...
class go_hor_dalaran_portal : public GameObjectScript
{
    public:
	go_hor_dalaran_portal() : GameObjectScript("go_hor_dalaran_portal") { }
